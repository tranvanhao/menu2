...
@objc public protocol SlideMenuOptionsDelegate {
    @objc optional func onMenuClick(indexPath: IndexPath)
}
...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    var slideMenuOptionsDelegate: SlideMenuOptionsDelegate?
    ...
    open func closeLeft(indexPath: IndexPath) {
        closeLeft()
        
        if let delegate = slideMenuOptionsDelegate, let onMenuClick = delegate.onMenuClick {
            onMenuClick(indexPath)
        }
    }
